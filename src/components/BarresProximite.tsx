...
export default function BarresProximite({
  ...
  return (
    <section className="space-y-3">
      <h2 className="text-h4">Proximité avec chaque plateforme</h2>
      ...
      <ul className="space-y-3">
        {ordre.map((sigle) => {
          ...
          return (
            <li key={sigle} className="rounded-[3px] border border-trait bg-surface p-3">
              <div className="flex flex-wrap items-baseline justify-between gap-x-4 gap-y-1">
                ...
                {r.affiche ? (
                  <span className="text-petit">
                    <span className="text-h4">{pct(r.arrondi as number)}</span>
                    <span className="text-encre-douce"> · sur {r.n} questions</span>
                  </span>
                ) : (
                  <span className="text-petit text-encre-douce">Trop peu de questions comparables ({r.n})</span>
                )}
              </div>
              {r.affiche && (
              ...
              {tete.has(sigle) && (
                <p className="mt-2 text-petit font-medium">
